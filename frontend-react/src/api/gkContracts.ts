...
export async function bindRequirementsToFunction(
  contractId: number,
  functionId: number,
  requirementIds: number[],
) {
  const { data } = await apiClient.post(
    `/contracts/${contractId}/functions/${functionId}/requirements/bind`,
    { requirementIds },
  )
  return data
}

export async function unbindRequirementsFromFunction(
  contractId: number,
  functionId: number,
  requirementIds: number[],
) {
  const { data } = await apiClient.post(
    `/contracts/${contractId}/functions/${functionId}/requirements/unbind`,
    { requirementIds },
  )
  return data
}
...
export async function downloadGKAttachment(attachmentId: number) {
  return apiClient.get(`/contracts/attachments/${attachmentId}/download`, {
    responseType: 'blob',
  })
}
